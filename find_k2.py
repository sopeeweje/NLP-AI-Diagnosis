...
def find_k(data, trials, k):
    ...
    for selected_k in np.array(clusters):
      ...
      for i in np.arange(trials):
        km = MiniBatchKMeans(n_clusters=selected_k, init='k-means++', n_init=10, init_size=3000, batch_size=3000, verbose=0, max_no_improvement=None)
        km.fit(data)
    ...
    plt.savefig('k_selection_sse.eps', format='eps')
